...
def vel_limits(linesfile):
    ...
    vbar = float(line.split()[8])
    f_sysabs.close()

    return neg,pos,ew,vbar
...
def velcut(linesfile, testing=0):
    ...
    if type(cell_z) is not np.float64:
        ...
        negVelLim,posVelLim,ewSysabs,vbar = vel_limits(linesfile)
        ...
        # Testing file
        testFile = '{0:s}.testvelcut'.format(linesfile)
        ...
        for i in range(0,len(cell_z)):
            ...
            if vpec>negVelLim and vpec<posVelLim:
                ...
                f_newlines.write(tests.format(cell_z[i],cell_N[i],cell_b[i],int(cell_ID[i]),vpec))
                testf.write(tests.format(cell_z[i],cell_N[i],cell_b[i],int(cell_ID[i]),vpec))
